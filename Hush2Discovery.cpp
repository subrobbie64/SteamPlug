...
HushDiscovery::HushDiscovery() : _discoveredHushDevice(0), _discoveredHushDeviceType(-1) {
	_discoveryCompletedEvent = System::CreateEventFlag();

	__hook(&CwclBluetoothManager::OnDeviceFound, &_wclBluetoothManager, &HushDiscovery::wclBluetoothManagerDeviceFound);
	__hook(&CwclBluetoothManager::OnDiscoveringCompleted, &_wclBluetoothManager, &HushDiscovery::wclBluetoothManagerDiscoveringCompleted);
	__hook(&CwclBluetoothManager::OnDiscoveringStarted, &_wclBluetoothManager, &HushDiscovery::wclBluetoothManagerDiscoveringStarted);

	__hook(&CwclGattClient::OnConnect, &_wclGattClient, &HushDiscovery::wclGattClientConnect);
	__hook(&CwclGattClient::OnDisconnect, &_wclGattClient, &HushDiscovery::wclGattClientDisconnect);
...
HushDiscovery::~HushDiscovery() {
	...
	__unhook(&CwclBluetoothManager::OnDeviceFound, &_wclBluetoothManager, &HushDiscovery::wclBluetoothManagerDeviceFound);
	__unhook(&CwclBluetoothManager::OnDiscoveringCompleted, &_wclBluetoothManager, &HushDiscovery::wclBluetoothManagerDiscoveringCompleted);
	__unhook(&CwclBluetoothManager::OnDiscoveringStarted, &_wclBluetoothManager, &HushDiscovery::wclBluetoothManagerDiscoveringStarted);

	__unhook(&CwclGattClient::OnConnect, &_wclGattClient, &HushDiscovery::wclGattClientConnect);
	__unhook(&CwclGattClient::OnDisconnect, &_wclGattClient, &HushDiscovery::wclGattClientDisconnect);

	System::DestroyEventFlag(_discoveryCompletedEvent);
}

CwclBluetoothRadio* HushDiscovery::getRadio() {
...
void HushDiscovery::wclBluetoothManagerDiscoveringStarted(void* Sender, CwclBluetoothRadio* const Radio) {
...
void HushDiscovery::wclBluetoothManagerDeviceFound(void* Sender, CwclBluetoothRadio* const Radio, const __int64 Address) {
...
bool HushDiscovery::runDiscovery(ButtplugConfig* config) {
...
void HushDiscovery::wclBluetoothManagerDiscoveringCompleted(void* Sender, CwclBluetoothRadio* const Radio, const int Error) {
...
void HushDiscovery::inspectNextDevice() {
	if (_discoveredDevices.empty()) {
...
void HushDiscovery::wclGattClientConnect(void* Sender, const int Error) {
...
void HushDiscovery::wclGattClientDisconnect(void* Sender, const int Reason) {
...
int HushDiscovery::getHushDeviceType(BtAddress address, wclGattServices& services) {
